(() => {
  ...
  const testGenSpinner = document.getElementById('test-gen-spinner');
  ...
  if (testGenBtn) {
    testGenBtn.addEventListener('click', async function () {
      const provider = apiProviderSelect.value;
      const apiBase = document.getElementById(`${provider}-api-base`).value;
      const modelSel = document.getElementById(`${provider}-model`).value;
      const model = modelSel === 'custom' ? document.getElementById(`${provider}-custom-model`).value : modelSel;
      if (!apiBase || !model) {
        showNotification('请输入基础URL与模型', 'warning');
        return;
      }
      const apiKey = await readApiKey(provider);
      if (!apiKey) {
        showNotification('缺少API密钥', 'warning');
        return;
      }
      testGenSpinner.classList.remove('hidden');
      testGenBtn.disabled = true;
      updateApiStatus('testing', '测试中...');
      const started = performance.now();
      try {
        const { url, headers, body } = buildChatRequest(provider, apiBase, model, apiKey, '请返回{"ok":true}');
        ...
        if (ok) {
          ...
          showNotification('生成测试成功', 'success');
        } else {
          throw new Error('生成响应解析失败');
        }
      } catch (err) {
        updateApiStatus('disconnected', '连接失败');
        showNotification(`生成测试失败：${err.message}`, 'error');
      } finally {
        testGenSpinner.classList.add('hidden');
